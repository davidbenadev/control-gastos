...
export function MonthlyDashboard({ initialMonth = monthKey(new Date()), expenses, subscriptions }: MonthlyDashboardProps) {
  ...
  return <main className="mx-auto w-full max-w-6xl px-5 py-10 sm:px-8">
    <header className="mb-10 flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-medium text-emerald-400">Control personal</p><h1 className="text-3xl font-bold tracking-tight text-slate-100">Resumen de gastos</h1></div><div className="flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-900 p-1 shadow-sm"><button aria-label="Mes anterior" onClick={() => setSelectedMonth(moveMonth(selectedMonth, -1))} className="rounded-lg px-3 py-2 text-slate-200 hover:bg-slate-800">←</button><span className="min-w-40 text-center text-sm font-semibold capitalize text-slate-200">{dateLabel(selectedMonth)}</span><button aria-label="Mes siguiente" onClick={() => setSelectedMonth(moveMonth(selectedMonth, 1))} className="rounded-lg px-3 py-2 text-slate-200 hover:bg-slate-800">→</button></div></header>
...
function Metric({ label, value, detail, accent = "text-slate-100" }: { label: string; value: number; detail: string; accent?: string }) { return <article className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-sm"><p className="text-sm text-slate-400">{label}</p><p className={`mt-2 text-2xl font-bold ${accent}`}>{money.format(value)}</p><p className="mt-1 text-xs text-slate-500">{detail}</p></article>; }
function MovementList({ title, empty, items }: { title: string; empty: string; items: { id: string; name: string; detail: string; amount: number }[] }) { return <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-sm"><h2 className="font-semibold text-slate-100">{title}</h2>{items.length === 0 ? <p className="py-8 text-sm text-slate-400">{empty}</p> : <ul className="mt-3 divide-y divide-slate-800">{items.map((item) => <li key={item.id} className="flex items-center justify-between gap-4 py-3"><div><p className="font-medium text-slate-200">{item.name}</p><p className="text-xs text-slate-400">{item.detail}</p></div><span className="font-semibold text-slate-100">{money.format(item.amount)}</span></li>)}</ul>}</section>; }
